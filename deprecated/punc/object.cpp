...
Object::Object(std::shared_ptr<df::FunctionSpace> &V,
...
void Object::compute_interpolated_charge(std::shared_ptr<df::Function> &q_rho)
{
                               ...
                               std::shared_ptr<const df::Mesh> &mesh)
{
    ...
    {
        flux->set_exterior_facet_domains(objects[j].bnd);
        flux->set_coefficient("w0", E);
...
VObject::VObject(const std::shared_ptr<df::FunctionSpace> &V,
                 std::shared_ptr<df::MeshFunction<std::size_t>> boundaries,
                 std::size_t bnd_id,
                 double potential,
                 double charge,
