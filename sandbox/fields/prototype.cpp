...
#include <chrono>
#include <dolfin.h>
#include "../../deprecated/UFL/Potential.h"
#include "../../deprecated/UFL/Potential.h"
#include "../../deprecated/UFL/Flux.h"
#include "../../deprecated/UFL/EField.h"
#include "../../deprecated/UFL/volume.h"
#include "../../deprecated/UFL/Energy.h"
...
public:
    ...
    Object(const Potential::FunctionSpace &V,
...
Object::Object(const Potential::FunctionSpace &V,
               const df::MeshFunction<std::size_t> &boundaries,
               std::size_t bnd_id,
               double potential,
               double charge,
               bool floating,
               std::string method):
               df::DirichletBC(std::make_shared<Potential::FunctionSpace>(V),
                               ...
                               const df::Mesh &mesh)
{
    ...
    Flux::Form_flux flux(std::make_shared<const df::Mesh>(mesh),
                         std::make_shared<const df::Function>(E));
...
PoissonSolver::PoissonSolver(const df::FunctionSpace &V,
                             ...
                             a(std::make_shared<df::FunctionSpace>(V),
                               std::make_shared<df::FunctionSpace>(V)),
                             L(std::make_shared<df::FunctionSpace>(V))
{
    df::assemble(A, a);
    if(ext_bc)
    {
...
{
public:
  std::shared_ptr<EField::FunctionSpace> W;
  df::PETScKrylovSolver solver;
  EField::BilinearForm a;
  EField::LinearForm L;
  df::PETScMatrix A;
  df::PETScVector b;
...
ESolver::ESolver(const df::FunctionSpace &V,
                 ...
                 solver(method, preconditioner), a(W,W), L(W)
{
    solver.parameters["absolute_tolerance"] = 1e-14;
    solver.parameters["relative_tolerance"] = 1e-12;
    solver.parameters["maximum_iterations"] = 1000;
...
double mesh_potential_energy(df::Function &phi, df::Function &rho)
{
    ...
    return 0.5 * df::assemble(energy_form);
}
...
int main()
{
    ...
    Potential::FunctionSpace V(mesh, constr);
