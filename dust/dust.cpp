    ...
    vector<string> distribution)
{
    ...
    // CREATE SOLVERS
    //
    PoissonSolver poisson(V, ext_bc, eps0);
    ESolver esolver(V);
    ...
    if(continue_simulation){
        ...
        file_hist << "#:xaxis\tt\n";
        file_hist << "#:name\tn\tt\tne\tni\tKE\tPE\tQ\tV\tI\n";
        file_hist << "#:long\ttimestep\ttime\t\"electron density\"\t";
        file_hist << "\"ion density\"\t\"kinetic energy\"\t";
        file_hist << "\"potential energy\"\tcharge\tvoltage\tcurrent\n";
        file_hist << "#:units\t1\ts\tm**(-3)\tm**(-3)\tJ\tJ\tC\tV\tA\n";
    }
    ...
    for(; n<=steps; ++n){
        ...
        auto phi = poisson.solve(rho, int_bc, V);
...
int main(int argc, char **argv){
    ...
    desc.add_options()
        ...
        ("species.Bx", po::value(&Bx), "magnetic field [T]")
